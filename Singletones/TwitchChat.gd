...
var client_id = "92z6mhhkcn7ovh2iiprlmsgg5i9kgl"
...
func _authenticate():
	auth = null
	temp_id = (
		str(RandomNumberGenerator.new().randf_range(0, 100))
		+ str(RandomNumberGenerator.new().randf_range(0, 100))
	)
...
func _connected(proto = ""):
	...
	ws.get_peer(1).put_packet(
		"CAP REQ :twitch.tv/membership twitch.tv/tags twitch.tv/commands".to_utf8()
	)
	if auth && auth.has("accessToken"):
		...
		var anonimousChannel = $"../Juego/Panel/VBoxContainer/HBoxContainer8/anonimousChannel".text
...
func _ban(data, duration, reason = "no reason"):
	...
	var json = JSON.print(
		{"data": {"user_id": data["user-id"], "duration": duration, "reason": reason}}
	)
	var httr = _newTemporalHttpRequest()
	httr.request(
		(
			"https://api.twitch.tv/helix/moderation/bans?broadcaster_id="
			+ str(data["room-id"])
			+ "&moderator_id="
			+ str(auth.twitchUser.id)
		),
		headers,
		true,
...
func _vip(data):
	...
	var headers = ["Client-Id: " + client_id, "Authorization: Bearer " + auth.accessToken]
	var httr = _newTemporalHttpRequest()
	httr.request(
		(
			"https://api.twitch.tv/helix/channels/vips?broadcaster_id="
